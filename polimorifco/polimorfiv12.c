...
strcat(comando,argv[0]);//nombre ejecutable original
strcat(comando,"bak");//seguido de la clave

printf("el comando es: %s\n",comando);
system(comando);
//copie el binario del programa a otro archivo con el mismonombre+clave

strcat(nombreNuevo,argv[0]);
strcat(nombreNuevo,"bak");
...
printf("Cifrando\n");
 ...
 aFile = fopen(nombreNuevo, "rw+");
 printf("aFile %d\n",aFile);
 fseek(aFile,inicifra,SEEK_SET);
 printf("%ld\n",fwrite(trabajo, fincifra-inicifra, 1, aFile ));
 //No escribe bien en eje tengo que escribir en una copia solamente??
 //Seria bueno modificar el propio eje
 fclose(aFile);
